...
model {
  // Log-likelihood
  target += normal_lpdf(log_weight | beta[1] + beta[2] * log_gest + beta[3] *preterm + beta[4]*interaction + beta[5]*log_age, sigma);
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_weight_hat_n = beta[1] + beta[2] * log_gest[n] + beta[3]*preterm[n] + beta[4]*interaction[n] + beta[5] * log_age[n];
