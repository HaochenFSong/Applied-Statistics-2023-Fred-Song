...
dm2 <- pop |>
  rename("Female_pop" = 'Female',
         ...
         "Total_pop" = 'Total')
```
now we can left join the table and calculate the mortality rates:
```{r}
...
Comment: Population-wise Male's average mortality rate is higher than Female's average mortality rate in Ontario, Canada. The general trend for both genders' mortality rate is decreasing before the year 2005, while slightly increasing after the year 2005. 
